...
def add_user(username, secret)
  unless @users_hash[username]
    ...
    File.open(file_path('users.yml', 'private'), 'w') do |f|
      f.write(@users_hash.to_yaml)
    end
  end
end
...
def valid_user?(username, password)
  @users_hash[username] &&
    BCrypt::Password.new(@users_hash[username]) == password
end
...
def reject_unsupported(index)
  ...
  index.select do |fn|
    supported.include?(File.extname(fn)) && !fn.start_with?('.')
  end.sort
end
...
post '/upload' do
  ...
  if !valid_filename_upload(filename)
    session[:messages] <<
      "Unsupported filetype. Supported filetypes: #{(SUPPORTED_DOCTYPES + SUPPORTED_IMAGETYPES).join(', ')}."
...
def write_file_history(filename, history)
  File.open(file_path(history_filename(filename)), 'w') do |f|
    f.write(history.to_yaml)
  end
end

def last_version(file_history)
  file_history[cur_time] = 'File history initialised.' if file_history.empty?
...
post '/:filename' do |filename|
  ...
  if new_file_ext == old_file_ext
    if new_filename != filename
      ...
      File.rename(
        file_path(history_filename(filename)),
        file_path(history_filename(new_filename)),
      )
    end
    if last_version(@file_history) != params[:new_content]
      File.open(file_path(new_filename), 'w') do |f|
...
post '/:filename/duplicate' do |filename|
  ...
  create_file(
    File.basename(filename, '.*') + ' copy' + File.extname(filename),
    file_content,
  )
